...
router.post('/', async function(req, res, next){
  if (req.body.lobbyName.length <= 3 || req.body.lobbyName.length >= 26 || req.body.lobbyPass.length >= 26 || !(/^$|^[A-Za-z0-9]+$/.test(req.body.lobbyPass)) ){
      ...
      if(req.header('Authorization') === myPlayer.token){
        ...
        Lobby.setId(lobbyId);
        let obj;
        if (req.body.lobbyPass === "" || req.body.lobbyPass === undefined || req.body.lobbyPass ===null){     //Creating Lobby with a password
            obj = {lobbyId: lobbyId, 
        ...
        } else{                                                                 //Creating Lobby without a password
            obj = {lobbyId: lobbyId,
               lobbyName: req.body.lobbyName, 
               hostName: myPlayer.playerName, 
               lobbyBoolPass: true, 
            ...
            if (myLobby.board[req.body.cell] === ' '){
              ...
              redisClient.hmset(`lobby#${req.params.id}`, obj, (err, result) => {
                if (result) {
                    ...
                    if (boolVictory ===true){
                      res.status(200).json({ status: 'ok', response:{board: board, gameResult: 'victory', player1Victories: obj.player1Victories, player2Victories: obj.player2Victories}});
                    } else{
                      if (nextTurn>=10){
                        res.status(200).json({ status: 'ok', response:{board: board, gameResult: 'draw', draws:obj.draws}});
                      } else{
                        res.status(200).json({ status: 'ok', response:{board: board, gameResult: 'stillPlaying'}});
                      } 
                    }
                } else {
                  res.status(400).json({status: 'error', response:{error: 'cantCreate'}});
